 ...
 */
abstract class CollectionQuery[CC[A] <: GenTraversable[A], A](companion: GenericCompanion[CC]) extends Query[CC[A]] {
  def map(row: Row): A
 ...
 */
abstract class FlatCollectionQuery[CC[A] <: GenTraversable[A], A](companion: GenericCompanion[CC]) extends Query[CC[A]] {
